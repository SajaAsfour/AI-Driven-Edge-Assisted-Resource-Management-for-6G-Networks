...
class MultiTrafficPredictor:
	...
	def run(self) -> MultiTrafficPredictionResult:
		...
		for dti_index in range(runtime_1.env.m):
			...
			requested_2 = runtime_2.env.infer_rb_from_traffic(
			...
			step_log = TrafficStepLog(
				...
				beta_current_1=float(eval_1["beta_current"]),
				beta_current_2=float(eval_2["beta_current"]),
			...
			self.logger.info(
				"DTI %s | request_1=%s request_2=%s total=%s scaled=%s | alloc_1=%s alloc_2=%s | beta_1=%.4f beta_2=%.4f | reward_1=%.4f reward_2=%.4f",
				display_dti,
				requested_1,
				requested_2,
